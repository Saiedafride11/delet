...
import { useAddPurchaseReturnMutation } from "../../../redux/features/purchase/return/purchaseReturnApi";
...
const PurchaseReturnCreate = () => {
  document.title = "Add Purchase Return";
  ...
  useEffect(() => {
    if (!purchaseIsLoading && initialPurchase !== undefined) {
      setFormValue((prevValue) => ({
        ...prevValue,
        party: initialPurchase?.party || {},
        party_id: initialPurchase?.party?.id || "",
        payment_type_id: initialPurchase?.payment_type_id || "",
        // paid_amount: initialPurchase?.paid_amount || 0,
        // due_amount: initialPurchase?.due_amount || 0,
        // total_amount: 0,
        // grand_total: initialPurchase?.grand_total || 0,

        paid_amount: 0,
        total_amount: 0,
        grand_total: 0,
        invoice_amount: {
          paidAmount: initialPurchase?.paid_amount || 0,
          dueAmount: initialPurchase?.due_amount || 0,
          grandTotal: initialPurchase?.grand_total || 0,
        },
  ...
  useEffect(() => {
    if (responseError !== undefined) {
      ...
      // navigate(
      //   `/purchase/return/invoice/${responseData?.data?.purchase_return.id}`
      // );
              ...
              <div className="col-sm-6 col-xl-4 mb-3">
                <div className="custom-focus-label">
                  <input
                    ...
                    value={
                      invoicePrefixData?.data?.["purchase-return-invoice"] || ""
                    }
                  />
                  <label>Invoice</label>
                </div>
