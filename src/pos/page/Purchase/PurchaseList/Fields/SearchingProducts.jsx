...
const SearchingProducts = ({ formValue, setFormValue }) => {
  ...
  // const filterQuery = `?search=${searchText}&per_page=30&filter=Purchase`;
  const filterQuery = `?search=${searchText}&per_page=30`;
  const {
    data: productsData,
    isLoading,
    isError,
  ...
  const handleProductClick = (data) => {
    ...
    if (isExist) {
      ...
      const totalAmount = formValue?.items?.reduce(
      ...
      setFormValue((prev) => ({
        ...
        items: [
          ...prev.items,
          {
            ...
            warehouse_id: formValue?.warehouse?.id,
            warehouse_name: formValue?.warehouse?.name,
