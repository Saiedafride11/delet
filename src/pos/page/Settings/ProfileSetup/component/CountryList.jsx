import React, { useEffect, useState } from "react";
import { useGetCountriesQuery } from "../../../../redux/features/global/countries/countriesApi";

const CountryList = ({ initialKey, initialValue, setFormValue }) => {
  const [searchText, setSearchText] = useState("");
  const [enabledResult, setEnabledResult] = useState(false);
  const [items, setItems] = useState([]);

  const {
    data: countriesData,
    isLoading,
    isError,
    isSuccess,
  } = useGetCountriesQuery();

  const initialData = countriesData?.data?.countries;

  useEffect(() => {
    if (searchText === "" || searchText === undefined) {
      setItems(initialData);
    } else {
      filterSearchItems(searchText);
    }
  }, [searchText, initialData]);

  useEffect(() => {
    if (initialValue !== "") {
      const matchedData = initialData?.find(
        (item) => item?.alpha3_code === initialValue
      );
      setSearchText(matchedData?.name);
      filterSearchItems(searchText);
    }
  }, [initialValue, initialData]);

  const handleOnChange = (e) => {
    const value = e.target.value;
    setSearchText(value);
    // filterSearchItems(value);

    if (value === "") {
      setFormValue((prev) => ({ ...prev, [initialKey]: "" }));
    }
  };

  const handleFieldClick = () => {
    setItems(initialData);
    setEnabledResult(true);
  };

  const handleResultClick = (code, name) => {
    setEnabledResult(false);
    setSearchText(name);
    setItems(initialData);
    setFormValue((prev) => ({ ...prev, [initialKey]: code }));
  };

  const filterSearchItems = (text) => {
    const matchedData = initialData?.filter((item) =>
      item?.name?.toLowerCase().includes(text?.toLowerCase())
    );
    setItems(matchedData || []);

    // console.log("text", text, matchedData);
  };
  return (
    <>
      <div className="input-wrapper pos-up-down-arrow">
        <div className="w-100">
          <div className="custom-focus-label">
            <input
              value={searchText || ""}
              onClick={handleFieldClick}
              onChange={handleOnChange}
              onBlur={() => setEnabledResult(false)}
              type="text"
              placeholder="Enter country"
              className="form-control m-0 search-select-option-input"
            />
            <label>Country</label>
          </div>
          <div
            className={`search-select-option ${
              enabledResult ? "d-block" : "d-none"
            }`}
          >
            {items?.length === 0 ||
            items === undefined ||
            isLoading ||
            isError ? (
              <div>
                <button className="option-btn text-center">
                  No data found!
                </button>
              </div>
            ) : (
              items?.map((item, i) => (
                <div key={i}>
                  <button
                    onMouseDown={() =>
                      handleResultClick(item?.alpha3_code, item?.name)
                    }
                    className="option-btn"
                  >
                    {item.name}
                  </button>
                </div>
              ))
            )}
            {/* when scroll pagination open, then show this loader */}
            {items !== undefined &&
              isLoading === false &&
              isSuccess === false && (
                <div className="bg-red">
                  <button className="option-btn text-center text-white fw-bold">
                    Loading...
                  </button>
                </div>
              )}
          </div>
        </div>
        <span></span>
      </div>
    </>
  );
};

export default CountryList;
