...
const IncomeCreateUpdate = ({
  ...
  useEffect(() => {
    if (updateData?.sector !== undefined) {
      setFormValue({
        income_date: updateData?.income_date?.split(" ")[0] || "",
        income_expense_category_id:
  ...
  const handleAddSubmit = (e) => {
    ...
    if (formValue?.income_expense_category_id === "") {
      ...
      addIncome({
        ...formValue,
        income_date: new Date(formValue?.income_date).toLocaleDateString(
          "en-GB"
        ),
      });
    }
  ...
  // console.log("updateData", updateData?.sector);
  return (
    <div className={`modal-content ${isCategoryOpen && "d-none"}`}>
      <div className="modal-header">
        <h1 className="modal-title fs-5 fw-bold" id="exampleModalLabel">
          {updateData?.sector ? "Update Income" : "Add New Income"}
        </h1>
        <button
          type="button"
          className="btn-close"
...
export default IncomeCreateUpdate;
