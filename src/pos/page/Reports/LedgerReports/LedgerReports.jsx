...
import { useGetAllLedgerQuery } from "../../../redux/features/ledger/ledgerApi";
...
import LedgerReportsPdf from "./LedgerReportsPdf";
import LedgerReportsTableUI from "./LedgerReportsTableUI";

const LedgerReports = () => {
  document.title = "Ledger Reports";
  ...
  const partyFilter =
    partyType === "" ? filterQuery : filterQuery + `&filter=${partyType}`;

  const { dateFilter, dateFilterParams, handleDateFilter } =
    useFilterFromToDate();

  const newFilterQuery = partyFilter
    ? partyFilter + `&${dateFilterParams}`
    : `?${dateFilterParams}`;
  ...
  const {
    ...
    isError,
    isFetching,
  } = useGetAllLedgerQuery(newFilterQuery);
  // } = useGetLedgerListQuery(`${newFilterQuery}&filter_report=Report`);
  ...
  const handleTablePdfDownload = () => {
    ...
    addCenteredText(`Ledger Reports`, 11, 14, 12);
    addCenteredText(`Data: ${getCurrentDateWithLastYear()}`, 7, 19, 9);

    autoTable(pdf, {
      html: "#ledger-report-table",
      headStyles: {
        fillColor: "#dedede",
        textColor: "#2e2e3e",
        lineColor: "#dedede",
      ...
      didParseCell: function (data) {
        ...
        data.cell.styles.halign = [3, 4, 5].includes(data.column.index)
          ? "center"
          : "left";
      },
        ...
        <div className="acnoo-dashboard-wrapper reports-wrapper">
          <div className="acnoo-dashboard-details-wrapper">
            <div className="details-header print-d-none">
              <div className="title">
                <h4 className="padY-2">Ledger Reports</h4>
              </div>
            </div>
            ...
            <div className="dashboard-details-table-wrapper">
              <div className="total-count-area mt-2">
                <div className="count-item light-blue">
                  <h5>${getNumberWithCommas(twoDigitFixed(0))}</h5>
                  <p>Retailer</p>
                </div>
