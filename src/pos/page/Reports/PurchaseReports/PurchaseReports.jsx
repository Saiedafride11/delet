...
import { useGetPurchasesQuery } from "../../../redux/features/purchase/purchase/purchaseApi";
...
import PurchaseReportsPdf from "./PurchaseReportsPdf";
import PurchaseReportsTableUI from "./PurchaseReportsTableUI";

const PurchaseReports = () => {
  document.title = "Purchase Reports";
  ...
  const {
    ...
    isError,
    isFetching,
  } = useGetPurchasesQuery(`${newFilterQuery}&filter_report=Report`);
  ...
  const tableData = purchasesData?.data?.purchase?.data?.map((data) => ({
    ...
    "Bill Amount": `$${getNumberWithCommas(twoDigitFixed(data.grand_total))}`,
    Paid: `$${getNumberWithCommas(twoDigitFixed(data.paid_all_amount))}`,
  ...
  const handleTablePdfDownload = () => {
    ...
    addCenteredText(`Purchase Reports`, 11, 14, 12);
    addCenteredText(`Data: ${getCurrentDateWithLastYear()}`, 7, 19, 9);

    autoTable(pdf, {
      html: "#purchase-report-table",
      headStyles: {
        fillColor: "#dedede",
        textColor: "#2e2e3e",
        lineColor: "#dedede",
      ...
      didParseCell: function (data) {
        ...
        data.cell.styles.halign = [3, 4, 5, 6, 7, 8, 9].includes(
          data.column.index
        )
          ? "center"
          : "left";
  ...
  const purchase_filter = purchasesData?.data?.purchase_filter_total?.[0];
              ...
              <div className="total-count-area mt-2">
                <div className="count-item light-blue">
                  <h5>
                    $
                    {getNumberWithCommas(
                      twoDigitFixed(purchase_filter?.purchase_total || 0)
                    )}
                  </h5>
                  <p>Total Purchase</p>
                </div>
                <div className="count-item light-green">
                  <h5>
                    $
                    {getNumberWithCommas(
                      twoDigitFixed(purchase_filter?.purchase_paid || 0)
                    )}
                  </h5>
                  <p>Total Paid</p>
                </div>
                <div className="count-item light-orange">
                  <h5>
                    $
                    {getNumberWithCommas(
                      twoDigitFixed(purchase_filter?.purchase_due || 0)
                    )}
                  </h5>
                  <p>Total Unpaid</p>
                </div>
