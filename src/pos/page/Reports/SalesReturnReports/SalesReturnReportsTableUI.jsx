...
const SalesReturnReportsTableUI = ({
  saleReturnData,
  isLoading,
  isError,
  page,
    ...
    <>
      ...
      <tbody>
        ...
        isError ? (
          ...
          saleReturnData?.data?.["sale-return"]?.data?.map((data, index) => (
            <tr key={index}>
              <td>
                <SerialNumberWithoutCheckbox
                  dataFrom={saleReturnData?.data?.["sale-return"]?.from}
                  index={index}
                />
              </td>
        ...
        {page === "pdf" && (
          <tr>
            ...
            <td className="fw-bold text-center">
              $
              {getNumberWithCommas(
                twoDigitFixed(saleReturnData?.data?.total_grand_total || 0)
              )}
            </td>
          </tr>
        )}
...
export default SalesReturnReportsTableUI;
